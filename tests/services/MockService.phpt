...
final class MockServiceTest extends TestCase {
    ...
    function setUp() {
        ...
        $this->class = $this->container->getByType('Impala\MockService');
        $this->tables = $this->mockRepository->getTestCollections();
        ...
        foreach ($parameters as $key => $annotations) {
            if (is_array($annotations)) {
                foreach($annotations as $table => $annotation) {
                    ...
                    $arguments = $this->class->getCall($annotation['service'], $annotation['method'], $parameters, $this->container->getByType('Impala\IEditFormFactory'));
