...
final class FilterFormTest extends TestCase {
    ...
    public function testAttached() {
        ...
        foreach ($this->presenters as $class => $latte) {
            ...
            Assert::true(is_object($impala = $this->container->getByType('Impala\Impala')), 'Impala is not set.');
            Assert::true(is_object($impala->setGrid($presenter->grid)), 'Impala:setGrid does not return class itself.');
            Assert::true(is_object($impala->setRow($presenter->row)), 'Impala::setRow does not return class itself.');
            Assert::true(is_object($presenter->addComponent($impala, 'Impala')), 'Attached Impala failed.');
            ...
            Assert::true($impala instanceof Impala, 'Impala has wrong instation.');
