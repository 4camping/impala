...
final class ReactFormTest extends TestCase {
    ...
    public function testAttached() {
        ...
        Assert::true(is_object($builder = $this->container->getByType('Impala\IRow')), 'IBuilder is not set.');
        Assert::false(empty($collections =  $this->container->parameters['collections']), 'No collections to test.');
        shuffle($collections);
        Assert::false(empty($collection = reset($collections)), 'Collection is not set.');
        foreach ($this->presenters as $class => $latte) {
            ...
            Assert::false(empty($data = $this->class->getData()), 'Form values are not set in class ' . $class . ' for source ' . $collection);
            ...
            Assert::true(property_exists($this->class, 'jsDir'), 'Private property for javascript does not exist.');
