...
final class ImportFormTest extends TestCase {
    ...
    public function testSetService() {
        ...
        foreach ($this->presenters as $class => $latte) {
            ...
            Assert::true(is_object($masala = $presenter->context->getByType('Impala\Impala')), 'Impala is not set.');
            Assert::true(is_object($masala->setGrid($presenter->grid)), 'Impala:setGrid does not return class itself.');
            Assert::true(is_object($presenter->addComponent($this->container->getByType('Impala\Impala'), 'masala')), 'Attached Impala failed.');                        
            Assert::true(is_object($masala = $presenter->getComponent('masala')), 'Impala is not set');
            Assert::same(null, $masala->attached($presenter), 'Impala:attached succeed but method return something. Do you wish to modify test?');
