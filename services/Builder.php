...
final class Builder implements IBuilder {
    ...
    public function attached(IImpalaFactory $impala): void {
        ...
        foreach ($this->columns as $column => $annotation) {
            if (preg_match('/\sAS\s/', $annotation)) {
                ...
                throw new InvalidStateException('Style, groups and class and className keywords are reserved for callbacks and column for Grid.jsx:update method. See https://github.com/landrisek/Impala/wiki/Select-statement. Use different alias.');
            }
    ...
    public function order(array $order): IBuilder {
        foreach($order as $column => $value) {
            if(!isset($this->columns[$column])) {
                throw new InvalidStateException('You muse define order column ' . $column . ' in select method of Impala\IBuilder.');
